...
/**
 	* A data access object (DAO) providing persistence and search support for Shoes entities.
...
public class ShoesDAO extends BaseHibernateDAO  {
    ...
    public void save(Shoes transientInstance) {
        ...
        try {
            ...
            System.out.println("存储时间：" + (endTime - startTime) + "ms");    //输出程序运行时间
    ...
    public Shoes findById( java.lang.String id) {
        ...
        try {
            Shoes instance = (Shoes) getSession()
                    .get("com.model.Shoes", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List findByExample(Shoes instance) {
        ...
        try {
            List results = getSession()
                    .createCriteria("com.model.Shoes")
    ...
    public String change(String name ,String val, Shoes instance){
        ...
        if(name.equals("innerMaterial"))
        	...
        	return ERROR;
        try {
        	instance.setUpdateTime(new Timestamp(new Date().getTime()));
        	Transaction tran = getSession().beginTransaction();
